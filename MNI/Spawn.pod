...
instance, C<RegisterPrograms> (subroutine) corresponds to
...
about with C<RegisterPrograms> (more about this below); if it is C<2>, then
...
Boolean; if true, then C<Spawn> might search a list of directories (see the
C<search_path> option below) to determine the location of the program it's
about to execute.  This won't happen if C<Spawn> already has an idea of the
program's location, either through a previous call to C<RegisterPrograms>
(which is how you tell C<Spawn> in advance what programs you expect to be
running) or if you supply a program name with directory components (either
absolute or relative).  Note that since searching is a sub-task of
completion, C<complete> must be true for C<search> to have any effect.
Also, don't assume that just because C<Spawn> doesn't search a list of
directories for your program, nobody will---Perl's C<exec> calls either the
shell or C<execvp(3)>, which means the PATH environment variable will be
searched.
...
C<MNI::FileUtilities::register_programs>, so the same rules apply as to the
...
is printed for each missing program and C<RegisterPrograms> returns false.
...
Only if all programs listed in PROGRAMS are found does C<RegisterPrograms>
...
See also C<register_programs>, C<find_program>, and C<search_directories> in
L<MNI::FileUtilities>.
